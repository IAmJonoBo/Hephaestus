...
function generateCliDoc(schemas) {
  ...
  doc.push(
    'description: "Complete command-line interface reference for Hephaestus toolkit"',
  );
  doc.push("---");
  doc.push("");
  ...
  doc.push("");
  doc.push(
    "The Hephaestus CLI provides commands for quality gates, refactoring analysis, and project automation.",
  );
  doc.push("");
  doc.push("## Commands");
  doc.push("");
...
function main() {
  ...
  // Export schemas
  const schemasJson = runCommand(
    "uv run hephaestus schema --output /tmp/schemas.json",
  );
