...
async def test_execute_guard_rails_synthesises_gates() -> None:
    ...
    # When required tooling is unavailable the guard-rails pipeline should
    # report the missing dependencies and mark the execution as unsuccessful.
    assert result["success"] is False
    assert any(gate for gate in result["gates"] if gate["metadata"].get("missing"))
